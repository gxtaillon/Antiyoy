...
public class GameController {
    ...
    private int possibleWinner() {
        ...
        if (zeroesInArray(playerHexCount) == colorNumber - 1)
            return indexOfNumberInArray(playerHexCount, maxNumberFromArray(playerHexCount));

        float allHexes = activeHexes.size();
        for (int i = 0; i < playerHexCount.length; i++) {
            float pHexes = playerHexCount[i];
            if (pHexes / allHexes > 0.8) {
                return i; // winner
            }
        }
        return -1;
